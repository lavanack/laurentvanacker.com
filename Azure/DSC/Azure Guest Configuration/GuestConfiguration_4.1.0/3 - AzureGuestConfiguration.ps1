...
$Params = @{
  ...
  "Path" =  './policies'
  "Platform" =  'Windows'
  "PolicyVersion" =  $([System.Version]::Parse("1.0.0"))
...
$Policy = Get-ChildItem "$CurrentDir\policies" | Where-Object -FilterScript { $_.name -eq "$($ConfigurationName)_DeployIfNotExists.json" -or $_.name -eq "$($ConfigurationName)_AuditIfNotExists.json" }
$Definition = New-AzPolicyDefinition -Name $ConfigurationName -Policy $Policy
