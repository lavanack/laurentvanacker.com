...
#region Administrative templates: DO NOT USE
function New-FSLogixIntuneAdministrativeTemplateConfigurationPolicyViaGraphAPI {
    ...
    switch ($FSLogixProfileContainersGroupPolicyDefinitions) {
        ...
        { $_.FullPath -eq '\FSLogix\Profile Containers\VHDLocations' } { Set-GroupPolicyDefinitionSettingViaGraphAPI -GroupPolicyConfiguration $GroupPolicyConfiguration -GroupPolicyDefinition $_ -Value "\\$CurrentHostPoolStorageAccountName.file.$StorageEndpointSuffix\profiles"; continue } 
        { $_.FullPath -eq '\FSLogix\Profile Containers\RedirXMLSourceFolder' } { Set-GroupPolicyDefinitionSettingViaGraphAPI -GroupPolicyConfiguration $GroupPolicyConfiguration -GroupPolicyDefinition $_ -Value "\\$CurrentHostPoolStorageAccountName.file.$StorageEndpointSuffix\profiles"; continue } 
...
function New-FSLogixIntuneSettingsCatalogConfigurationPolicyViaGraphAPI {
    ...
    #Adding a FullPath Property
    if ($FSLogixConfigurationChildCategories.Count -gt 0) {
        $FSLogixConfigurationChildCategories = foreach ($CurrentFSLogixConfigurationChildCategory in $FSLogixConfigurationChildCategories) {
            #Adding a FullPath Property
            $FullPath = Set-CategoryFullPath -Categories $FSLogixConfigurationChildCategories -CurrentCategory $CurrentFSLogixConfigurationChildCategory
            $CurrentFSLogixConfigurationChildCategory | Select-Object -Property *, @{Name = "FullPath"; Expression = { $FullPath } }
        }
    }
    #endregion
    ...
    [array] $settings += switch ($FSLogixProfileContainersConfigurationSettings) {
        ...
        { $_.FullPath -eq '\FSLogix\Profile Containers\VHDLocations' } { New-IntuneSettingsCatalogConfigurationPolicySettingsViaGraphAPI -Settings $FSLogixProfileContainersConfigurationSettings -Setting $_ -SettingValue "\\$HostPoolStorageAccountName.file.$StorageEndpointSuffix\profiles" -Enable; continue } 
        { $_.FullPath -eq '\FSLogix\Profile Containers\RedirXMLSourceFolder' } { New-IntuneSettingsCatalogConfigurationPolicySettingsViaGraphAPI -Settings $FSLogixProfileContainersConfigurationSettings -Setting $_ -SettingValue "\\$HostPoolStorageAccountName.file.$StorageEndpointSuffix\profiles" -Enable; continue } 
        default { Write-Verbose -Message "'$($_.FullPath)' not modified" }  
    }
    #endregion
    ...
    #Adding a FullPath Property
    if ($WindowsUpdateForBusinessConfigurationChildCategories.Count -gt 0) {
        $WindowsUpdateForBusinessConfigurationChildCategories = foreach ($CurrentWindowsUpdateForBusinessConfigurationChildCategory in $WindowsUpdateForBusinessConfigurationChildCategories) {
            #Adding a FullPath Property
            $FullPath = Set-CategoryFullPath -Categories $WindowsUpdateForBusinessConfigurationChildCategories -CurrentCategory $CurrentWindowsUpdateForBusinessConfigurationChildCategory
            $CurrentWindowsUpdateForBusinessConfigurationChildCategory | Select-Object -Property *, @{Name = "FullPath"; Expression = { $FullPath } }
        }
    }
    #endregion
    ...
    if ($AdministrativeTemplatesConfigurationChildCategories.Count -gt 0) {
        #Adding a FullPath Property
        $AdministrativeTemplatesConfigurationChildCategories = foreach ($CurrentAVDConfigurationChildCategory in $AdministrativeTemplatesConfigurationChildCategories) {
            #Adding a FullPath Property
            $FullPath = Set-CategoryFullPath -Categories $AdministrativeTemplatesConfigurationChildCategories -CurrentCategory $CurrentAVDConfigurationChildCategory
            $CurrentAVDConfigurationChildCategory | Select-Object -Property *, @{Name = "FullPath"; Expression = { $FullPath } }
        }
    }
    #endregion
    ...
    #Adding a FullPath Property
    if ($StorageConfigurationChildCategories.Count -gt 0) {
        $StorageConfigurationChildCategories = foreach ($CurrentStorageConfigurationChildCategory in $StorageConfigurationChildCategories) {
            #Adding a FullPath Property
            $FullPath = Set-CategoryFullPath -Categories $StorageConfigurationChildCategories -CurrentCategory $CurrentStorageConfigurationChildCategory
            $CurrentStorageConfigurationChildCategory | Select-Object -Property *, @{Name = "FullPath"; Expression = { $FullPath } }
        }
    }
    #endregion
...
function New-FSLogixIntuneAdministrativeTemplateConfigurationPolicyViaCmdlet {
    ...
    switch ($FSLogixProfileContainersGroupPolicyDefinitions) {
        ...
        { $_.FullPath -eq '\FSLogix\Profile Containers\VHDLocations' } { Set-GroupPolicyDefinitionSettingViaCmdlet -GroupPolicyConfiguration $GroupPolicyConfiguration -GroupPolicyDefinition $_ -Value "\\$CurrentHostPoolStorageAccountName.file.$StorageEndpointSuffix\profiles"; continue } 
        { $_.FullPath -eq '\FSLogix\Profile Containers\RedirXMLSourceFolder' } { Set-GroupPolicyDefinitionSettingViaCmdlet -GroupPolicyConfiguration $GroupPolicyConfiguration -GroupPolicyDefinition $_ -Value "\\$CurrentHostPoolStorageAccountName.file.$StorageEndpointSuffix\profiles"; continue } 
        default { Write-Verbose -Message "'$($_.FullPath)' not modified" }  
    }
    #endregion
...
function Wait-AzVMPowerShell {
    ...
    Do {
        ...
        $Jobs = foreach ($CurrentSessionHost in $SessionHosts) {
            ...
            $null = Invoke-AzVMRunCommand -ResourceGroupName $ResourceGroupName -VMName $CurrentSessionHostVM.Name -CommandId 'RunPowerShellScript' -ScriptString 'return $true' -AsJob
        }
        $Jobs | Wait-Job | Out-Null
...
function New-AzAvdPersonalHostPoolSetup {
    ...
    process {
        Foreach ($CurrentHostPool in $HostPool) {
            ...
            $Tag = @{HostPoolName = $CurrentHostPool.Name; HostPoolType = [HostPoolType]::Personal; Intune = $Status[$CurrentHostPool.Intune]; CreationTime = [datetime]::Now; CreatedBy = (Get-AzContext).Account.Id }
...
'@
    ...
    process {
        Foreach ($CurrentHostPool in $HostPool) {
            ...
            $Status = @{ $true = "Enabled"; $false = "Disabled" }
            $Tag = @{MSIX = $Status[$CurrentHostPool.MSIX]; FSLogix = $Status[$CurrentHostPool.FSLogix]; Intune = $Status[$CurrentHostPool.Intune]; HostPoolName = $CurrentHostPool.Name; HostPoolType = [HostPoolType]::Pooled; CreationTime = [datetime]::Now; CreatedBy = (Get-AzContext).Account.Id }
            if ($CurrentHostPool.VMSourceImageId) {
                $Tag['Image'] = 'Azure Compute Gallery'
                $Tag['VMSourceImageId'] = $CurrentHostPool.VMSourceImageId
            }
            ...
            if (($CurrentHostPool.IsMicrosoftEntraIdJoined()) -and ($CurrentHostPool.FSLogix)) {
                ...
                if (-not($CurrentHostPool.Intune)) {
                    foreach ($CurrentSessionHostName in $SessionHostNames) {
                        ...
                        $null = Invoke-AzVMRunCommand -ResourceGroupName $CurrentHostPoolResourceGroupName -VMName $CurrentSessionHostName -CommandId 'RunPowerShellScript' -ScriptPath $ScriptPath
                        ...
                        $ScriptPath = Join-Path -Path $env:Temp -ChildPath $(Split-Path -Path $URI -Leaf)
                        Invoke-WebRequest -Uri $URI -UseBasicParsing -OutFile $ScriptPath
                        $null = Invoke-AzVMRunCommand -ResourceGroupName $CurrentHostPoolResourceGroupName -VMName $CurrentSessionHostName -CommandId 'RunPowerShellScript' -ScriptPath $ScriptPath -Parameter @{CurrentHostPoolStorageAccountName = $CurrentHostPoolStorageAccountName }
                        Remove-Item -Path $ScriptPath -Force
                        #endregion
                        #endregion
                    ...
                    #region AVD Global Settings FSLogix
                    #region AVD Global Settings FSLogix - Intune Configuration Profile
                    ...
                    #region Configure FSLogix
                    #region Configure FSLogix - Intune Configuration Profile
    ...
    process {
        ...
        if ($AsJob) {
            ...
            $ExportedFunctions = [scriptblock]::Create(@"
                ...
                Function New-AzAvdSessionHost { ${Function:New-AzAvdSessionHost} }
                Function New-FSLogixIntuneAdministrativeTemplateConfigurationPolicyViaCmdlet { ${Function:New-FSLogixIntuneAdministrativeTemplateConfigurationPolicyViaCmdlet} }  
                Function New-AzAvdIntuneAdministrativeTemplateConfigurationPolicyViaCmdlet { ${Function:New-AzAvdIntuneAdministrativeTemplateConfigurationPolicyViaCmdlet} }  
