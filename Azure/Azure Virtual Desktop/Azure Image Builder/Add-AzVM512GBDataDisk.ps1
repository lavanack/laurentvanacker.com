...
function Get-AzAccessToken {
    ...
    try {
        $AccessToken = Invoke-RestMethod -Uri $uri -Headers @{"Metadata" = "true" } -Method GET -TimeoutSec 5
        Write-Verbose -Message "[$(Get-Date -Format "yyyy-MM-dd HH:mm:ss")][$($MyInvocation.MyCommand)] Leaving function '$($MyInvocation.MyCommand)'"
        Write-Verbose -Message "[$(Get-Date -Format "yyyy-MM-dd HH:mm:ss")][$($MyInvocation.MyCommand)] Azure Access Token:`r`n$($AccessToken | Out-String)"
        return $AccessToken
    }
    catch {
        Write-Verbose -Message "[$(Get-Date -Format "yyyy-MM-dd HH:mm:ss")][$($MyInvocation.MyCommand)] Leaving function '$($MyInvocation.MyCommand)'"
...
#$AccessToken = Invoke-RestMethod -Uri "http://169.254.169.254/metadata/identity/oauth2/token?api-version=2019-08-01&resource=https://management.azure.com/" -Method GET -Headers @{Metadata="true"} -TimeoutSec 5
$AccessToken = Get-AzAccessToken -Verbose
Connect-AzAccount -AccessToken $AccessToken.access_token -AccountId $AccessToken.client_id
#endregion

#region Variable definitions
$ThisVM = Get-AzVMCompute -Verbose | Get-AzVM
