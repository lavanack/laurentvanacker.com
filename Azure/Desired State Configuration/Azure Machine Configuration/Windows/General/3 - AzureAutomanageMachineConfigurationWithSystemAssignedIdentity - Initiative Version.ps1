...
#region Our Guest Policies will be put in an Initiative (Policy Set)
...
#region Creating the policies to put in the Initiative (Policy Set)
$policySetMembers = @()
...
#region Creating the Initiative
$FileName = "AzureAutomanageMachineConfigurationInitiative_{0:yyyyMMddHHmmss}.json" -f $(Get-Date)
$FilePath = Join-Path -Path $env:TEMP -ChildPath $FileName
$policySetMembers | ConvertTo-Json -Depth 100 | Set-Content -Path $FilePath
$Metadata = @{ category = "Guest Configuration" } | ConvertTo-Json -Depth 100 
$Name =(New-Guid).Guid   
$DisplayName = "[$ResourceGroupName][Windows] Make sure all Windows servers comply with DSC Configurations"
#Display Name is limited to 128 characters 
$DisplayName = $DisplayName.Substring(0, [math]::min(128, $DisplayName.Length))
$Params = @{
    Name             = $Name
    DisplayName      = $DisplayName
    Description      = "Aggregated Guest Configuration Policies"
    PolicyDefinition = $FilePath
    Metadata         = $Metadata
    #Verbose          = $true
}
$PolicySetDefinition = New-AzPolicySetDefinition @Params
#endregion
