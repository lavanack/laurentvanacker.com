...
$HDDDisks = ($StoppedVM.StorageProfile.OsDisk | Get-AzDisk) | Where-Object -FilterScript { $_.Sku.Name -eq "Standard_LRS"}
if ($StoppedVM.StorageProfile.DataDisks) {
    $HDDDisks += ($StoppedVM.StorageProfile.DataDisks | Get-AzDisk) | Where-Object -FilterScript { $_.Sku.Name -eq "Standard_LRS"}
}

foreach ($CurrentHDDDisk in $HDDDisks)
{
    Write-Output -InputObject "`$CurrentHDDDisk : $($CurrentHDDDisk.Name)"
    $CurrentHDDDisk.Sku = [Microsoft.Azure.Management.Compute.Models.DiskSku]::new("StandardSSD_LRS")
