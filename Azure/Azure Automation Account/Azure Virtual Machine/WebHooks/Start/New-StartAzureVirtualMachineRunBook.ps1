...
while (-not $rbacSuccess -and $rbacRetryCount -lt $maxRbacRetries) {
    ...
    try {
        $roleAssignment = New-AzRoleAssignment -ObjectId $AutomationAccount.Identity.PrincipalId -RoleDefinitionName 'Virtual Machine Contributor' -Scope "/subscriptions/$SubscriptionId" -ErrorAction Stop
        Write-Host "Successfully assigned 'Virtual Machine Contributor' role to managed identity" -ForegroundColor Green
...
$RunBookName = "{0}-StartAzureVirtualMachine" -f $RunBookPrefix
...
if (-not $SkipTesting) {
    ...
    try {
        ...
        $testVMs = Get-AzVM -Status | Where-Object -FilterScript { $_.PowerState -ne "VM running" }
        
        if ($testVMs.Count -eq 0) {
            Write-Warning "No stopped VMs found for testing. Skipping webhook test."
        }
        else {
            # Select up to 2 VMs for testing
            $selectedVMs = $testVMs | Get-Random -Count ([Math]::Min(2, $testVMs.Count)) | Select-Object -Property Name, ResourceGroupName
            
            Write-Host "Selected VMs for testing:" -ForegroundColor Cyan
            $selectedVMs | ForEach-Object { 
                Write-Host "  - $($_.Name) (RG: $($_.ResourceGroupName))" -ForegroundColor White 
            }
