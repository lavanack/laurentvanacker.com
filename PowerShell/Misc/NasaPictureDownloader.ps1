...
Function Get-PlanetaryNasaPictures
{
	...
	ForEach ($CurrentURI in $URI)
	{
		...
		While ($HasMorePictures)
		{
			...
			$Links=($Response.Links | Where-Object {$_.href -like 'http://www.planetary.org/multimedia/space-images/*.html'})
...
Function Download-NasaPictures
{
	...
	end
	{
		if ($Asynchronous)
		{
			...
			#http://www.jonathanmedd.net/2013/04/start-bitstransfer-submitting-greater-than-60-asynchronous-jobs-generates-error.html
			#http://www.computerstepbystep.com/limit-the-maximum-number-of-bits-jobs-for-each-user.html
			#http://www.computerstepbystep.com/limit-the-maximum-number-of-files-allowed-in-a-bits-CurrentBitsTransfer.html#PowerShellScript
			$BitsTransferred = 0
			$BitsError = 0
			$Index=0
				...
				while ($BitsTransfers.Count -ge $MaxJobsPerUser)
				{
					...
					Foreach($CurrentBitsTransfer in $BitsTransfers)
					{
						Switch($CurrentBitsTransfer.JobState)
						{
							'Transferring' { break }
							'Connecting' { break }
							'Transferred' { $BitsTransferred++; $CurrentContentLength+=$CurrentBitsTransfer.BytesTotal; Complete-BitsTransfer -BitsJob $CurrentBitsTransfer; break }
							'Error' { Write-Host -Object "Error while downloading $($CurrentBitsTransfer.FileList.RemoteName) ..." -ForegroundColor Red; $CurrentBitsTransfer | Remove-BitsTransfer ; $BitsError++; break }
							default {Write-Host -Object "Other action while downloading $($CurrentBitsTransfer.FileList.RemoteName) : $($CurrentBitsTransfer.ErrorDescription) ..." -ForegroundColor Red; $CurrentBitsTransfer | Remove-BitsTransfer ; $BitsError++; break}
						}
					}
...
$Missions=@{
	...
	'NASA Image of the Day'='http://www.nasa.gov/api/1/query/ubernodes.json?unType[]=image&routes[]=1446&page=[PAGENUMBER]&pageSize=24';
}
...
$Planetaries = @{
	'The Planetary Society'='http://www.planetary.org/multimedia/space-images/index.jsp?page=[PAGENUMBER]';
}

$SDOs = @{
	'Solar Dynamics Observatory'='http://sdo.gsfc.nasa.gov/gallery/main';
}

$Assets=@{
	'Recents'='http://images-assets.nasa.gov/recent.json';
	'Popular'='http://images-assets.nasa.gov/popular.json';
}

$JPLs = @{
	'Jet Propulsion Laboratory'='http://www.jpl.nasa.gov/assets/json/getMore.php?images=true&page=[PAGENUMBER]'
}
...
ForEach ($CurrentSDO in $SDOs.Keys)
{
	...
	$NasaPictures+=Get-SDONasaPictures -Verbose -Domain 'http://sdo.gsfc.nasa.gov' -URI $SDOs[$CurrentSDO] -Destination $DestinationFolder
} 
